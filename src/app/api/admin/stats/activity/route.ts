/**
 * Admin Activity Statistics API
 * GET /api/admin/stats/activity - Get user activity statistics
 */

import { NextResponse } from 'next/server';
import { requireAdminAPI } from '@/lib/admin/middleware';
import { getActivityStats, getTopUsers } from '@/lib/admin/stats';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Get activity stats
    const [activity, topUsersCredits, topUsersSearches] = await Promise.all([
      getActivityStats(fromDate, toDate),
      getTopUsers('credits_used', 10),
      getTopUsers('searches', 10),
    ]);

    return NextResponse.json({
      activity,
      top_users: {
        by_credits: topUsersCredits,
        by_searches: topUsersSearches,
      },
    });
  } catch (err) {
    console.error('Error fetching activity stats:', err);
    return NextResponse.json(
      { error: 'Fehler beim Laden der Aktivitätsstatistiken' },
      { status: 500 }
    );
  }
}
