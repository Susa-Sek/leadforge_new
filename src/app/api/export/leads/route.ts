...
// Export leads from search results (Enterprise feature)
...
export async function POST(request: Request) {
  try {
    ...
    let countQuery = supabase.from('search_results').select('*', { count: 'exact', head: true }).eq('user_id', user.id);
    ...
    // 7. Validate against plan limits (leads = bulk export = Enterprise)
    ...
    if (rowCount === 0) {
      return NextResponse.json(
        { error: 'Keine Leads zum Exportieren gefunden. Passen Sie Ihre Filter an.', code: 'NO_DATA' },
        { status: 404 }
      );
    }
    ...
    console.error('Error in leads export:', error);
    return NextResponse.json(
